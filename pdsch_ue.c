        ...
        case DECODE_PDSCH:
          ...
          if (decode_pdsch) {
            ...
            if (n > 0) {
              ...
              /* add MAC_LTE header */
              MAC_Context_Info_t capInfo;
			  capInfo.radioType = ue_dl.cell.frame_type + 1;
              capInfo.direction = DIRECTION_DOWNLINK;
			  capInfo.rnti = pdsch_cfg.rnti;

			  if(pdsch_cfg.rnti != SRSLTE_SIRNTI) {
			    capInfo.rntiType = C_RNTI;
			  }
			  else{
			  	capInfo.rntiType = SI_RNTI;
			  }
			  if (acks[0]){
			      capInfo.ueid = pdsch_cfg.rnti;    
			  }
			  else
			  {
			      capInfo.ueid = pdsch_cfg.rnti - 1;  
			  }
			  capInfo.crcStatusOK = true;
			  
			  capInfo.sysFrameNumber = sfn;
			  capInfo.subFrameNumber = sf_idx;

              if (prog_args.net_port > 0) {
                if (sf_idx == 1) {
                  //srslte_netsink_write(&net_sink, data[0], 1 + (n - 1) / 8);
				  srslte_netsink_write_pcap(&capInfo,&net_sink, data[0], 1 + (n - 1) / 8);
                } else {
                  // FIXME: UDP Data transmission does not work
                  for (uint32_t tb = 0; tb < SRSLTE_MAX_CODEWORDS; tb++) {
                    if (pdsch_cfg.grant.tb[tb].enabled) {
					  //srslte_netsink_write(&net_sink, data[tb], 1 + (pdsch_cfg.grant.tb[tb].tbs - 1) / 8);
                      srslte_netsink_write_pcap(&capInfo,&net_sink, data[tb], 1 + (pdsch_cfg.grant.tb[tb].tbs - 1) / 8);
                    }
                  }
                }
              }
